...
?>
<!doctype html>
<html lang="es-AR" class="h-full" x-data="AAPPAppAqua()" x-init="init()">
<head>
  <meta charset="utf-8" />
  ...
  <script defer>
    window.AAPPAppAqua = function AAPPAppAqua() {
      ...
      return {
        ...
        startClock() {
          const update = () => {
            const now = new Date();
            this.aquaClockTime = now.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
            this.aquaClockDate = now.toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
          };
          update();
  ...
  <style>
    ...
    .ink-blob.one { width: 420px; height: 420px; background: radial-gradient(circle at 30% 30%, rgba(59,130,246,.35), rgba(14,165,233,.08)); top: -120px; left: -80px; }
    .ink-blob.two { width: 360px; height: 360px; background: radial-gradient(circle at 70% 40%, rgba(94,234,212,.28), rgba(14,165,233,.06)); bottom: 0; right: -100px; }
    .glass {
      background: linear-gradient(135deg, rgba(255,255,255,.18), rgba(255,255,255,.08));
      border: 1px solid var(--glass-border);
      box-shadow: 0 12px 40px rgba(0,0,0,.32), inset 0 1px 0 rgba(255,255,255,.16);
      backdrop-filter: blur(16px) saturate(140%);
    }
    .glass-strong {
      background: radial-gradient(circle at 10% 10%, rgba(255,255,255,.14), transparent 38%), linear-gradient(180deg, rgba(255,255,255,.12), rgba(255,255,255,.04));
      border: 1px solid rgba(255,255,255,.24);
      box-shadow: 0 15px 40px rgba(0,0,0,.35), inset 0 1px 0 rgba(255,255,255,.28);
      backdrop-filter: blur(18px) saturate(160%);
    }
    .ring-blue-soft { box-shadow: 0 0 0 1px rgba(56,189,248,.30), 0 18px 35px rgba(14,165,233,.18); }
    .scrollbar::-webkit-scrollbar{ height:10px; width:10px; }
    .scrollbar::-webkit-scrollbar-thumb{ background: rgba(56,189,248,.35); border-radius: 999px; }
    .scrollbar::-webkit-scrollbar-track{ background: rgba(255,255,255,.06); }
    .badge {
...
<body
  class="h-full text-slate-100 relative overflow-x-hidden"
  x-cloak
  :style="{
  ...
  <div class="ink-blob two -z-10"></div>
  <div class="absolute inset-0 -z-20 bg-gradient-to-br from-sky-900/30 via-slate-900/60 to-slate-950"></div>
  <div
    x-show="appLoading"
      ...
      <header class="sticky top-0 z-40 border-b border-white/10 bg-slate-950/80 backdrop-blur">
        <div class="w-full max-w-none px-4 py-4 flex flex-wrap items-center gap-3">
          ...
          <div class="flex items-center gap-2 ml-auto flex-wrap justify-end">
            ...
            <button class="btn rounded-xl px-3 py-2 text-sm flex items-center gap-2" @click="aquaHomeOpen = !aquaHomeOpen">
              <i class="fa-solid fa-house text-sky-300"></i>
          ...
          <div class="glass-strong rounded-3xl p-6 ring-blue-soft relative overflow-hidden">
            ...
            <div class="relative z-10">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 mt-6">
                <button
                  class="tile rounded-2xl p-4 text-left hover:border-sky-400/60 hover:shadow-lg transition"
                  @click="activeTab = 'dashboard'; aquaHomeOpen = false">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-xs text-sky-200/80">Panel</p>
                      <h3 class="text-lg font-extrabold">Dashboard</h3>
                    </div>
                    <span class="w-10 h-10 rounded-xl glass flex items-center justify-center">
                      <i class="fa-solid fa-gauge-high text-sky-200"></i>
                    </span>
                  </div>
                  <p class="mt-2 text-sm text-slate-300">KPIs y salud general.</p>
                </button>

                <button
                  class="tile rounded-2xl p-4 text-left hover:border-sky-400/60 hover:shadow-lg transition"
                  @click="activeTab = 'tasks'; aquaHomeOpen = false">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-xs text-sky-200/80">Tareas</p>
                      <h3 class="text-lg font-extrabold">Flujo operativo</h3>
                    </div>
                    <span class="w-10 h-10 rounded-xl glass flex items-center justify-center">
                      <i class="fa-solid fa-list-check text-sky-200"></i>
                    </span>
                  </div>
                  <p class="mt-2 text-sm text-slate-300">Checklist y progreso.</p>
                </button>

                <button
                  class="tile rounded-2xl p-4 text-left hover:border-sky-400/60 hover:shadow-lg transition"
                  @click="activeTab = 'clients'; aquaHomeOpen = false">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-xs text-sky-200/80">Clientes</p>
                      <h3 class="text-lg font-extrabold">Directorio</h3>
                    </div>
                    <span class="w-10 h-10 rounded-xl glass flex items-center justify-center">
                      <i class="fa-solid fa-users text-sky-200"></i>
                    </span>
                  </div>
                  <p class="mt-2 text-sm text-slate-300">Accesos rápidos a fichas.</p>
                </button>

                <button
                  class="tile rounded-2xl p-4 text-left hover:border-sky-400/60 hover:shadow-lg transition"
                  @click="activeTab = 'notes'; aquaHomeOpen = false">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-xs text-sky-200/80">Notas</p>
                      <h3 class="text-lg font-extrabold">Tablero</h3>
                    </div>
                    <span class="w-10 h-10 rounded-xl glass flex items-center justify-center">
                      <i class="fa-solid fa-note-sticky text-sky-200"></i>
                    </span>
                  </div>
